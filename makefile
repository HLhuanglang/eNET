...
easy_net: premake
	...
	@$(MAKEF) COMM_MODE=LIB \
		...
		COMM_DEFINES=DEBUG	\
		COMM_INCDIRS="$(EASYNET_INC_DIRS)" \
		LIB_TARGET_NAME=$@ \
		LIB_TYPE="SHARED|STATIC" \
		LIB_OBJS="$(EASYNET_OBJS)" \
		LIB_SRCS_DIR=${ROOT_DIR}/easy_net \
		LIB_OBJS_DIR=${BUILD_OBJS_DIR}\
		LIB_OUTDIR=${BUILD_EASYNET_DIR}/lib
...
test: easy_net
	...
	@$(MAKEF) COMM_MODE=EXE\
		...
		EXE_SRCS_DIR=${ROOT_DIR}/test\
		EXE_TARGET_DIR=${BUILD_BIN_DIR}/test\
		EXE_TARGET="$(TEST_TARGET)"\
