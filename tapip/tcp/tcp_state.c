...
static void tcp_listen(struct pkbuf *pkb, struct tcp_segment *seg,
    ...
    // 服务端：[socket-->bind-->listen-->accept阻塞住]这一段都是listen状态
    // 处在listen状态的服务端只能接受来自客户端的syn报文
    tcpsdbg("2. check ack");
    if (tcphdr->ack) {
...
static void tcp_synsent(struct pkbuf *pkb, struct tcp_segment *seg,
                        struct tcp_sock *tsk) {
    // 客户端调用connect[此时发发送syn报文]到connect返回这段时间内,都是syn_sent状态
    struct tcp *tcphdr = seg->tcphdr;
    tcpsdbg("SYN-SENT");
...
void tcp_process(struct pkbuf *pkb, struct tcp_segment *seg, struct sock *sk) {
    ...
    struct tcp *tcphdr = seg->tcphdr;
    tcp_dbg_state(tsk);
    if (!tsk || tsk->state == TCP_CLOSED)  // tcp_sock默认状态是0，如果为null说明按照5元组并没找到tcp连接，需要关闭
