...
static struct sock *tcp_lookup_sock_establish(unsigned int src, unsigned int dst,
    ...
    struct sock *sk;
    head = tcp_ehash_head(tcp_ehashfn(src, dst, src_port, dst_port));  // 先拿到establish列表的对头
...
struct sock *tcp_lookup_sock(unsigned int src, unsigned int dst,
    ...
    // tcp连接5元组：协议+目标地址+port+源地址+port
    struct sock *sk;
    sk = tcp_lookup_sock_establish(src, dst, src_port, dst_port);  // 当前socket是否已经完成三次握手了
    if (!sk)
