...
int main(int argc, char* argv[])
{
    ...
    str_http_req += "Accept: */*\r\n";
    str_http_req += "Content-Length: 140\r\n";
    ...
    // settings.on_chunk_header = on_chunk_header_cb;
    ...
    http_parser_init(&parser, http_parser_type::HTTP_BOTH);
    //str_http_req += "{\"DeviceID\":\"12345678\",\"LowPressure\":80,\"HighPressure\":123,\"Pulse\":12}";
    ret_bytes = http_parser_execute(&parser, &settings, str_http_req.c_str(), str_http_req.size());
    std::cout << "ret_bytes=" << ret_bytes << " size=" << str_http_req.size() << std::endl;

    // http_parser_init(&parser, http_parser_type::HTTP_BOTH);
    // ret_bytes = http_parser_execute(&parser, &settings, str_http_res.c_str(), str_http_res.size());
